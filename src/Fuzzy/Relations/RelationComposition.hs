...
circlet (LRelation r u) (LRelation s _ ) = LRelation composition u
    where composition (x, z) = foldr (\/) bot [r (x, y) `tnorm` s (y, z) | y <- universe, (x, y) `elem` u]
...
subproduct (LRelation r u) (LRelation s _) = LRelation composition u
    where composition (x, z) = foldr (/\) top [r (x, y) --> s (y, z) | y <- universe, (x, y) `elem` u]
          universe = universeToList u
...
superproduct (LRelation r u) (LRelation s _) = LRelation composition u
    where composition (x, z) = foldr (/\) bot [r (x, y) <-- s (y, z) | y <- universe, (x, y) `elem` u]
          eq x y = r (x, y) <--> s (x, y)
          universe = universeToList u
